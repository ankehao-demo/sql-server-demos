-- PostgreSQL equivalent of [DataLoadSimulation].ProcessCustomerPayments
...
DECLARE
    ...
    v_customer_payment_type_id INTEGER;
...
BEGIN
    ...
    -- Get transaction type ID for customer payment
    SELECT transaction_type_id INTO v_customer_payment_type_id
    FROM application.transaction_types
    WHERE transaction_type_name = 'Customer Payment Received';
    ...
    -- Process payments for customers with outstanding balances
    -- Only process a random subset of customers each day
    FOR v_customer_id, v_outstanding_balance IN
        ...
        ORDER BY RANDOM()
        LIMIT FLOOR(RANDOM() * 20 + 5)::INTEGER
    LOOP
        ...
        VALUES (
            v_customer_id, v_customer_payment_type_id, NULL, v_eft_payment_method_id,
            p_current_date_time::DATE, 0 - v_outstanding_balance, 0, 0 - v_outstanding_balance,
            0, p_current_date_time::DATE, v_staff_member_person_id, p_starting_when
        );
...
COMMENT ON PROCEDURE dataload_simulation.process_customer_payments IS 'Simulates processing of customer payments';
